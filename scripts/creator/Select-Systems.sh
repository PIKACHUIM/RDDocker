...
if [ $OS_TYPE == 1 ]; then
  source scripts/creator/Ubuntu-Version.sh
  OS_TYPE=ubuntu
  OS_UPPE=Ubuntu
elif [ $OS_TYPE == 2 ]; then
  source scripts/creator/Debian-Version.sh
  OS_TYPE=debian
  OS_UPPE=Debian
#elif [ $OS_TYPE == 3 ]; then
#  source scripts/creator/ArchOS-Version.sh
#  OS_TYPE=archos
#  OS_UPPE=ArchOS
elif [ $OS_TYPE == 3 ]; then
  source scripts/creator/Alpine-Version.sh
  OS_TYPE=alpine
  OS_UPPE=Alpine
elif [ $OS_TYPE == 4 ]; then
  source scripts/creator/Fedora-Version.sh
  OS_TYPE=fedora
  OS_UPPE=Fedora
fi
